...
class Block:
    ...
    def __drawBox(self,DisplaySurf,x, y, color, pixelx=None, pixely=None):          #private method
        ...
        pygame.draw.rect(DisplaySurf, GRAY, (pixelx, pixely, BOXSIZE - 1, BOXSIZE - 1))
        pygame.draw.rect(DisplaySurf, BLUE, (pixelx +1, pixely+1, BOXSIZE - 5, BOXSIZE - 5))

    def drawBoard(self,DisplaySurf,board):
        pygame.draw.rect(DisplaySurf, WHITE, (SideMargin - 3, TopMargin - 7, (BWIDTH * BOXSIZE) + 8, (BHEIGHT * BOXSIZE) + 8), 5)
        pygame.draw.rect(DisplaySurf, BLACK, (SideMargin, TopMargin, BOXSIZE * BWIDTH, BOXSIZE * BHEIGHT))
    ...
    def drawPiece(self,DisplaySurf,piece, pixelx=None, pixely=None):
        ...
        for x in range(TEMPWIDTH):
            for y in range(TEMPHEIGHT):
                if shape[y][x] != BLANK:
                    self.__drawBox(DisplaySurf,None, None, GRAY, pixelx + (x * BOXSIZE), pixely + (y * BOXSIZE))
