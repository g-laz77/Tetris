...
class Gameplay(Board, Block):  # inheritance
    ...
    def on_init(self):
        ...
        self.DisplaySurf = pygame.display.set_mode(
            (self.__WWIDTH, self.__WHEIGHT), pygame.HWSURFACE)
    ...
    def on_execute(self):
        ...
        while self.__running:
                ...
                if event.type == KEYUP:
                    ...
                    if event.key == K_a and self.checkPiecePos(self.board, self.currentPiece, adjX=-1):
                        ...
                        self.currentPiece['rotation'] = (
                            self.currentPiece['rotation'] + 1) % len(PIECES[self.currentPiece['shape']])
                        if not self.checkPiecePos(self.board, self.currentPiece):
                            self.currentPiece['rotation'] = (
                                self.currentPiece['rotation'] - 1) % len(PIECES[self.currentPiece['shape']])
            ...
            self.drawNextPiece(
                self.DisplaySurf, self.nextPiece, self.BASICFONT)
            if self.currentPiece != None:
    ...
    def setmyvar(self, var):
        self.board = var

    def getval(self, board, num):
        return self.__checkRowFull(num)
